...
void GPIO_portEn(uint8_t port){
    switch(port){
        ...
        case PORTB:     APB_Enable(APB2, GPIOB_APB);     break;
        case PORTC:     APB_Enable(APB2, GPIOC_APB);     break;
    }
}
...
void GPIO_setDir(uint8_t port, uint8_t pin, uint8_t mode){
	...
	if(pin > 7)		GPIO[port].CRH &= ~(0x03 << ((pin-8)*4 + MODE_OFFSET));
	else			GPIO[port].CRL &= ~(0x03 << (pin*4 + MODE_OFFSET));

	if(mode != INPUT){
		if(pin > 7)	GPIO[port].CRH |= (0x01 << ((pin-8)*4 + MODE_OFFSET));
...
void GPIO_setOutputMode(uint8_t port, uint8_t pin, uint8_t mode){
	...
	if(pin > 7)		GPIO[port].CRH &= ~(0x03 << ((pin-7)*4 + CNF_OFFSET));
	else			GPIO[port].CRL &= ~(0x03 << (pin*4 + CNF_OFFSET));

	//---set bits
	if(pin > 7)		GPIO[port].CRH |= (mode << ((pin-7)*4 + CNF_OFFSET));
...
/*!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>!*/
/*!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>!*/
/* Para setear o resetear un pin se usa el registro de atomic set. Cada pin tiene asociados 2 bits.
